...
def moveMark():
   ...
   mag_angle = adjust_north(compass.run_compass_tilt(mag, accel, hardiron_cal)[0], 70)
   ...
   """
   if look_x >= x_boundary + overflow - 400:
...
#   if (marker_x <= marker_size) or (marker_x >= x_boundary - marker_size):
      xspeed = 0
   ...
   if ((marker_size - edge_margin <= marker_x <= x_boundary - marker_size + edge_margin) and not (look_x - 20 < marker_x < look_x + 20)):
