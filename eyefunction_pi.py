...
### This version is for use with the AR display (no Pi). To test how the display looks when using the object recognition plus computer vision.
...
def object_identifier():
	...
	frame_skip = 5  # Process every Xth frame
	frame_count = 0
	...
	while True:
			...
			if fixation_norm_pos != "None":
				...
				if y < HUDy:
					if condition_met_time1 is None:
						...
						print('waiting for 1 second to elapse')

					elif current_time - condition_met_time1 >= 1:
						# Condition has been met for 1 second
						if HUD == 1:
							HUD = 0
						else:
							HUD = 1
		...
		if topic == "frame.world":
			...
			# Below get frame for real-time video
		...
		if condition_met_time2 is not None and current_time - condition_met_time2 > 1.5:
			write_csv('eyefunction.csv', [[HUD,' ',label_loc]])
