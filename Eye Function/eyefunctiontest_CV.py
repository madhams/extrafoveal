...
def object_identifier():
	...
	# Dictionary for label location
	loc_dict = {0:(380,300), 1:(400,500), 2:(700,500), 3:(700,300), 4:(700,100), 5:(400,100), 6:(100,100), 7:(100,300), 8:(100,500)}
	...
	while True:
		...
		if topic == "fixations":
			...
			if fixation_norm_pos != "None":
				...
				if y < 0:
					...
					elif current_time - condition_met_time1 < 1.5:
		...
		if HUD == 1:
			if len(dist[dist>0]) > 0:
				...
				# print(min_bounds)
				if fixation[0] > min_bounds[0]+20 and fixation[0] < min_bounds[2]+20 and fixation[1] > min_bounds[1]+20 and fixation[1] < min_bounds[3]+20:
			...
			if condition_met_time2 is None:
				cv2.putText(frame, 'n/a', loc_dict[0], cv2.FONT_HERSHEY_SIMPLEX, 1, COLORS[0], 2)
			elif current_time - condition_met_time2 < 1.5:

				cv2.putText(frame, fixation_save, loc_dict[label_loc], cv2.FONT_HERSHEY_SIMPLEX, 1, COLORS[0], 2)
			else:
				cv2.putText(frame, fixation_save, loc_dict[label_loc], cv2.FONT_HERSHEY_SIMPLEX, 1, COLORS[0], 2)
				condition_met_time2 = None
			...
			if fixation_label == last_fixation_label and fixation_label != 'n/a':
				...
				# 	print('waiting')
					...
					# write text file, send it
					# scp = SCPClient(ssh.get_transport())
					if fixation[0] < 450 and fixation[0] > 350 and fixation[1] < 340 and fixation[1] > 260:
						label_loc = 0
					elif fixation[0] < 450 and fixation[0] > 350 and fixation[1] >= 340:
						label_loc = 1
					elif fixation[0] >= 450 and fixation[1] >= 340:
						label_loc = 2
					elif fixation[0] >= 450 and fixation[1] < 340 and fixation[1] > 260:
						label_loc = 3
					elif fixation[0] >= 450 and fixation[1] <= 260:
						label_loc = 4
					elif fixation[0] < 450 and fixation[0] > 350 and fixation[1] <= 260:
						label_loc = 5
					elif fixation[0] < 350 and fixation[1] <= 260:
						label_loc = 6
					elif fixation[0] < 350 and fixation[1] < 340 and fixation[1] > 260:
						label_loc = 7
					elif fixation[0] < 350 and fixation[1] > 340:
						label_loc = 8

					cv2.putText(frame, fixation_label, loc_dict[label_loc], cv2.FONT_HERSHEY_SIMPLEX, 1, COLORS[0], 2)
					# write_csv('eyefunction.csv', [[HUD,fixation_label,label_loc]])
